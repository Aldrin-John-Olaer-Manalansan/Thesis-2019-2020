...
class FileHandler():
    def __init__(self,push='bridge/userdata.json',pull='bridge/queue.json', backup='bridge/userdata.json.backup'):
        dirname = os.path.dirname(__file__)
        self.input=os.path.join(dirname, pull)
        self.output=os.path.join(dirname, push)
        self.backup=os.path.join(dirname, backup)
        self.timestamp = 0
        self.timeout = 0
        self.expected = []
        self.fetch()
...
#--------------Separator for Readability--------------#
    ...
    def save(self, notification='none'):
        ...
        with open(self.backup, 'w+') as backfile:
            print('saving backup...' + self.backup)
            json.dump(self.object, backfile)
    ...
    def fetch(self):
        ...
        try:
            ...
            try:
                with open(self.backup) as string:
                    self.object = json.loads(string.read())
                    self.object['timestamp']=self.timestamp
                self.serial = []
                for appliance in self.object['appliance']:
...
#--------------Separator for Readability--------------#
    ...
    def do(self, queue):
        ...
        if queue['request'] == 'register':
            ...
            print(queue)
            if queue['data']=='appliance':
                self.expected.append(queue['serial'])
            print('New Socket on queue: ' + str(self.expected))
...
UserData.addSocket([106, 199, 90, 177, 114])
UserData.addSocket([126, 199, 90, 177, 112])
