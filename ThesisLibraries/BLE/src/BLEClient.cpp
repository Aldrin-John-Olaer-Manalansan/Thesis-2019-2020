...
// and then block on its completion.  When the event has arrived, we will have the handle.
	...
	m_semaphoreRegEvt.take(5000,"connect");
	...
	m_semaphoreRegEvt.wait("connect",10000/portTICK_PERIOD_MS);

	m_peerAddress = address;

	// Perform the open connection request against the target BLE Server.
	m_semaphoreOpenEvt.take(5000,"connect");
	...
	uint32_t rc = m_semaphoreOpenEvt.wait("connect",10000/portTICK_PERIOD_MS);   // Wait for the connection to complete.
...
int BLEClient::getRssi() {
	...
	//
	m_semaphoreRssiCmplEvt.take(5000,"getRssi");
	...
	int rssiValue = m_semaphoreRssiCmplEvt.wait("getRssi",10000/portTICK_PERIOD_MS);
...
// TODO implement retrieving services from cache
	...
	m_semaphoreSearchCmplEvt.take(5000,"getServices");
	if (errRc != ESP_OK) {
	...
	m_haveServices = (m_semaphoreSearchCmplEvt.wait("getServices",10000/portTICK_PERIOD_MS) == 0);
	log_v("<< getServices");
	return &m_servicesMap;
} // getServices
