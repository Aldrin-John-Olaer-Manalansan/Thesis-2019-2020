...
void BLEService::executeCreate(BLEServer* pServer) {
	...
	m_pServer          = pServer;
	m_semaphoreCreateEvt.take(5000,"executeCreate"); // Take the mutex and release at event ESP_GATTS_CREATE_EVT
	...
	m_semaphoreCreateEvt.wait("executeCreate",10000/portTICK_PERIOD_MS);
	log_v("<< executeCreate");
} // executeCreate
...
void BLEService::executeDelete() {
	log_v(">> executeDelete()");
	m_semaphoreDeleteEvt.take(5000,"executeDelete"); // Take the mutex and release at event ESP_GATTS_DELETE_EVT
	...
	m_semaphoreDeleteEvt.wait("executeDelete",10000/portTICK_PERIOD_MS);
	log_v("<< executeDelete");
} // executeDelete
...
//
	...
	m_semaphoreStartEvt.take(5000,"start");
	...
	m_semaphoreStartEvt.wait("start",pdMS_TO_TICKS(5000));
...
// obtained as a result of calling esp_ble_gatts_create_service().
	...
	m_semaphoreStopEvt.take(5000,"stop");
	...
	m_semaphoreStopEvt.wait("stop",10000/portTICK_PERIOD_MS);
