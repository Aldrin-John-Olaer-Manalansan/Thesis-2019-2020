...
BLEService* BLEServer::createService(BLEUUID uuid, uint32_t numHandles, uint8_t inst_id) {
	...
	m_semaphoreCreateEvt.take(5000,"createService");
	...
	m_semaphoreCreateEvt.wait("createService",10000/portTICK_PERIOD_MS);
...
void BLEServer::registerApp(uint16_t m_appId) {
	...
	m_semaphoreRegisterAppEvt.take(5000,"registerApp"); // Take the mutex, will be released by ESP_GATTS_REG_EVT event.
	::esp_ble_gatts_app_register(m_appId);
	m_semaphoreRegisterAppEvt.wait("registerApp",10000/portTICK_PERIOD_MS);
	log_v("<< registerApp");
} // registerApp
